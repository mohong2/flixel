...
class FlxSprite extends FlxObject
{
	...
	override public function draw():Void
	{
			...
			&& FlxG.renderTile && !pixelPerfectPosition && cameras.length == 1)
		{
			...
			{
				final sx = x - cam.scroll.x * scrollFactor.x - offset.x;
				final sy = y - cam.scroll.y * scrollFactor.y - offset.y;

				@:privateAccess
				{
					if (sx + width > cam.viewOffsetX && sx < cam.viewOffsetWidth
						&& sy + height > cam.viewOffsetY && sy < cam.viewOffsetHeight)
					{
						_flashPoint.setTo(sx, sy);
						cam.copyPixels(_frame, framePixels, _flashRect, _flashPoint, colorTransform, blend, antialiasing);
						#if FLX_DEBUG
	...
	{
		var result:Bool = (angle == 0 || bakedRotationAngle > 0) && scale.x == 1 && scale.y == 1 && !flipX && !flipY && blend == null;
