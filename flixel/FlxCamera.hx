...
class FlxCamera extends FlxBasic
{
	...
	var _targetIsSprite:Bool = false;
	var _dzRight:Float = 0;
	var _dzBottom:Float = 0;
	...
	public function updateFollow():Void
	{
		...
		else
		{
			...
			else
			{
				...
				edge = targetX + target.width - _dzRight;
				if (_scrollTarget.x < edge)
				{
					_scrollTarget.x = edge;
				}
				...
				edge = targetY + target.height - _dzBottom;
				if (_scrollTarget.y < edge)
				{
					_scrollTarget.y = edge;
				}
	...
	public function follow(Target:FlxObject, ?Style:FlxCameraFollowStyle, ?Lerp:Float):Void
	{
		...
		target = Target;
		followLerp = Lerp;
		_targetIsSprite = (Target != null && Std.isOfType(Target, FlxSprite));
		var helper:Float;
		var w:Float = 0;
		var h:Float = 0;
		...
		if (deadzone != null)
		{
			_dzRight = deadzone.x + deadzone.width;
			_dzBottom = deadzone.y + deadzone.height;
			if (_lastTargetPosition == null && target != null)
	 ...
	 */
	public function setFilters(afilters:Array<BitmapFilter>):Void
	{
		filters = afilters;
	}
