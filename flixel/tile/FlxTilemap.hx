...
class FlxTilemap extends FlxBaseTilemap<FlxTile>
{
	...
	/**
	 * Prepares the tilemap for rendering. Handles buffer refresh if needed.
	 */
	override public function prepareForRender():Void
	{
	...
	function drawTilemap(Buffer:FlxTilemapBuffer, Camera:FlxCamera):Void
	{
		...
		var startDrawX = _helperPoint.x + screenXInTiles * scaledWidth;
		drawY = _helperPoint.y + screenYInTiles * scaledHeight;
		...
		for (row in 0...screenRows)
		{
			...
			for (column in 0...screenColumns)
			{
				...
				if (tile != null && tile.visible && tile.frame.type != FlxFrameType.EMPTY)
				{
					...
					{
						// drawX/drawY tracked incrementally
