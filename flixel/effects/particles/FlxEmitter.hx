	...
	public function emitParticle():T
	{
		...
		if (velocity.active)
		{
			...
			if (launchMode == FlxEmitterMode.CIRCLE)
			{
				...
				var _tempPt = FlxVelocity.velocityFromAngle(particleAngle, FlxG.random.float(speed.start.min, speed.start.max));
				_point.copyFrom(_tempPt);
				_tempPt.put();
				...
				_tempPt = FlxVelocity.velocityFromAngle(particleAngle, FlxG.random.float(speed.end.min, speed.end.max));
				_point.copyFrom(_tempPt);
				_tempPt.put();
