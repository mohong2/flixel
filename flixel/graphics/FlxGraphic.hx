...
class FlxGraphic implements IFlxDestroyable
{
	...
	public static function fromBitmapData(Source:BitmapData, Unique:Bool = false, ?Key:String, Cache:Bool = true):FlxGraphic
	{
		...
		if (graphic != null)
		{
			...
			{
				// The cached graphic is a destroyed zombie (auto-freed but key remains):
				// replace it with the fresh BitmapData instead of returning a dead graphic.
				FlxG.bitmap.removeByKey(key);
				graphic = null;
			}
			else
			{
				if (Source != null && Source != graphic.bitmap)
					Source.dispose(); // Key already cached: free the newly decoded bitmap.
				return graphic;
			}
		}
	...
	{
		// Safety net: a destroyed graphic has `frameCollections == null`;
		// recreate the map instead of crashing so stale cached references
		// degrade gracefully.
		if (frameCollections == null)
			frameCollections = new Map<FlxFrameCollectionType, Array<Dynamic>>();
	...
	{
		if (_imageFrame == null && bitmap != null)
