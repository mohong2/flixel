...
class FlxMouse extends FlxPointer implements IFlxInputManager
{
	...
	// [TEST] Drag-to-wheel: when enabled, dragging the mouse while holding
	// the left button will simulate mouse wheel events, enabling swipe/scroll
	// via click-and-drag on any existing FlxG.mouse.wheel logic.
	// This is a controlled experiment — set to true to enable.
	public var dragToWheelEnabled:Bool = false;
	var _dragLastScreenY:Int = -1;
	var _dragAccumulated:Int = 0;
	var _dragWheelLastTime:Float = 0;
	 ...
	 * In this version, screen space is the same as view space.
	 */
	...
	function update():Void
	{
		...
		// [TEST] Drag-to-wheel: when left button is pressed and dragToWheelEnabled,
		// accumulate vertical movement and emit wheel events with rate limiting.
		if (dragToWheelEnabled && _leftButton.pressed)
		{
			if (_dragLastScreenY < 0)
			{
				_dragLastScreenY = screenY;
				_dragAccumulated = 0;
				_dragWheelLastTime = -1;
			}
			else
			{
				var dy:Int = screenY - _dragLastScreenY;
				_dragAccumulated += dy;
				if (Math.abs(_dragAccumulated) >= 15)
				{
					// Rate limit: at least 80ms between simulated wheel events
					var now:Float = haxe.Timer.stamp();
					if (_dragWheelLastTime < 0 || (now - _dragWheelLastTime) >= 0.08)
					{
						wheel = (_dragAccumulated > 0) ? -1 : 1;
						// NOTE: do NOT set _wheelUsed here — the wheel reset logic
						// at the top of update() clears wheel each frame when
						// _wheelUsed is false. Drag sets wheel fresh each trigger
						// frame, so the next frame will correctly clear it to 0.
						_dragWheelLastTime = now;
					}
					_dragLastScreenY = screenY;
					_dragAccumulated = 0;
				}
