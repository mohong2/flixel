...
class FlxObject extends FlxBasic
{
	...
	function updateMotion(elapsed:Float):Void
	{
		...
		var vx = velocity.x;
		velocityDelta = 0.5 * (FlxVelocity.computeVelocity(vx, acceleration.x, drag.x, maxVelocity.x, elapsed) - vx);
		vx += velocityDelta;
		var delta = vx * elapsed;
		vx += velocityDelta;
		velocity.x = vx;
		x += delta;

		var vy = velocity.y;
		velocityDelta = 0.5 * (FlxVelocity.computeVelocity(vy, acceleration.y, drag.y, maxVelocity.y, elapsed) - vy);
		vy += velocityDelta;
		delta = vy * elapsed;
		vy += velocityDelta;
		velocity.y = vy;
	...
	public function overlaps(ObjectOrGroup:FlxBasic, InScreenSpace:Bool = false, ?Camera:FlxCamera):Bool
	{
		...
		var r = (objectScreenPos.x + object.width > _point.x)
	...
	/**
	 * Hook called by the renderer before drawing this object (tile renderer).
	 * Added to the base class so subclasses that override it (FlxTilemap,
	 * FlxBitmapText) can call `super.prepareForRender()`.
	 */
