...
class FlxGame extends Sprite
{
	...
	/**
	 * Optional external callback to wrap each `draw()` call.
	 * If set, the callback receives the draw function as an argument.
	 * The callback must call the passed function for drawing to occur.
	 *
	 * This allows project code to offload drawing to another thread,
	 * e.g. via `RenderThread.submitRender()`.
	 *
	 * Usage:
	 * ```haxe
	 * FlxG.game.drawWrapper = function(drawFn:Dynamic) {
	 *     RenderThread.submitRender(drawFn);
	 * };
	 * ```
	 */
	public var drawWrapper:Dynamic = null;
	...
	function set_separateUpdateDraw(value:Bool):Bool
	{
	...
	function create(_):Void
	{
		...
		if (FlxG.updateFramerate < FlxG.drawFramerate && !separateUpdateDraw)
	...
	// ═══════════════════════════════════════════════════════════════
	//  Separate Update/Draw mode management
	// ═══════════════════════════════════════════════════════════════

	/**
	 * Kept for API compatibility. Separate mode no longer uses a timer:
	 * updates are driven by the fixed-timestep accumulator in `onEnterFrame`,
	 * so no restart is needed when framerates change.
	 */
	public function startSeparateUpdateTimer():Void
	{
		// no-op (timer removed)
	}

	/**
	 * Kept for API compatibility. See `startSeparateUpdateTimer`.
	 */
	public function stopSeparateUpdateTimer():Void
	{
		// no-op (timer removed)
	}

	/**
	 * Kept for API compatibility. Exceptions now propagate through the normal
	 * `ENTER_FRAME` event like combined mode, so no timer crash recovery is needed.
	 */
	public var onTimerCrashCallback:String->String->Void;
	 ...
	 * If `drawWrapper` is set, the actual draw work is wrapped through it
	 * (e.g. to offload to a render thread).
	 */
	function draw():Void
	{
	...
	/**
	 * Internal method that performs the actual draw work.
	 * Extracted from `draw()` so it can be wrapped by `drawWrapper`.
	 */
